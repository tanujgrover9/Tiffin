...
export default function CartPage(): JSX.Element {
  ...
  const coupons: Coupon[] = [
    ...
    },
    {
      ...
      value: 50,
      minSubtotal: 200,
      desc: "₹50 off on UPI payments over ₹200",
    },
    {
      id: "WELCOME10",
      label: "WELCOME10",
  ...
  const discountAmount = useMemo(
    () => calcDiscount(appliedCoupon, subtotal),
  ...
  useEffect(() => {
    ...
    if (
      upi &&
      subtotal >= (upi.minSubtotal ?? 0) &&
      (!appliedCoupon || appliedCoupon.id !== upi.id)
    ) {
  ...
  const handleApplyCoupon = (): void => {
    ...
    if (found.minSubtotal && subtotal < found.minSubtotal) {
      toast.error(
        `Requires minimum subtotal of ${formatINR(found.minSubtotal)}`
      );
      return;
    }
    ...
    setCouponInput("");
    toast.success(
      `Coupon ${found.label} applied — saved ${formatINR(
        calcDiscount(found, subtotal)
      )}`
    );
  };
  ...
  const applySuggested = (c: Coupon) => {
    setAppliedCoupon(c);
    toast.success(
      `${c.label} applied — saved ${formatINR(calcDiscount(c, subtotal))}`
    );
  };
  ...
  const freeDeliveryProgress = Math.min(
    100,
    Math.round(
      (Math.min(subtotal, FREE_DELIVERY_THRESHOLD) /
        ...
        <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-8">
          ...
          <div className="space-y-6">
            <div className="flex items-start justify-between">
              {/* 🔁 Replaced "Your Cart" with Box badge design */}
            ...
            {items.length === 0 ? (
              <div className="border rounded-xl p-8 text-center text-gray-600">
                ...
                <div className="mt-4">
                  <Link
                    to="/menu"
                    className="inline-block px-4 py-2 bg-amber-500 text-white rounded-lg"
                  >
                    Browse Menu
                  </Link>
                </div>
                      ...
                      <div className="flex items-center gap-4 w-full md:w-auto">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-sm text-gray-500">
                            {formatINR(item.dish.price)} each
                          </p>
                      ...
                      <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                        <div className="flex items-center border rounded-md overflow-hidden">
                          <button
                            ...
                            onClick={() =>
                              handleQtyChange(item.dish.id, item.qty - 1)
                            }
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-600">
                          10% off with XYZ Bank Credit Card
                        </div>
                      </div>
                    </motion.div>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-600">
                          ₹50 cashback on UPI payments above ₹200
                        </div>
                      </div>
                    </motion.div>
                  </div>
                    ...
                    <div className="text-sm text-amber-700 flex items-center justify-between bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
                      <div>
                        ...
                        <span className="font-medium">
                          {suggestedCoupon.label}
                        </span>{" "}
                        — {suggestedCoupon.desc}
                      </div>
                      <button
          ...
          {showModal && (
            <>
              ...
              <motion.div
                ...
                dragElastic={0.3}
                onDragEnd={(_, info) =>
                  info.point.y > 200 && setShowModal(false)
                }
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  {/* Also use Box badge in mobile sheet header */}
                  <span className="inline-flex">
                    <span
                      className={`
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={handleProceedToCheckout}
                      className="flex-1 py-3 rounded-full bg-orange-500 text-white font-semibold"
                    >
                      Proceed to Checkout
                    </button>
                    <Link
                      to="/menu"
                      className="flex-1 py-3 rounded-full border text-center text-gray-700"
                    >
                      Continue Shopping
                    </Link>
                  </div>
